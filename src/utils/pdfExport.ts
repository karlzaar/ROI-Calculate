...
export function generatePDFReport(options: PDFExportOptions): void {
  ...
  // Debug info: show schedule data received
  const scheduleLen = data.payment.schedule?.length || 0;
  const lastDate = scheduleLen > 0 ? data.payment.schedule[scheduleLen - 1].date : 'none';
  doc.text(`Schedule: ${scheduleLen} entries, last: ${lastDate}`, pageWidth - margin, pageHeight - 8, { align: 'right' });

  // Save with debug info in filename
  const projectName = data.property.projectName || 'Investment';
  const fileName = `BaliInvest_${projectName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}_sch${scheduleLen}.pdf`;
  doc.save(fileName);
}
